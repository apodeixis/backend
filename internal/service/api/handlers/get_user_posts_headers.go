...
func GetUserPostsHeaders(w http.ResponseWriter, r *http.Request) {
	request, err := requests.NewGetUserPostsHeaders(r)
	if err != nil {
	...
	if request.Status != nil {
		postsQ.FilterByStatus(*request.Status)
	}
	postsHeaders, err := postsQ.Limit(request.Limit).FilterByUserID(request.UserID).SelectHeaders()
	if err != nil {
		ctx.Log(r).Error(err)
