/*
Apodeixis backend

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 0.1.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package resources

import (
	"bytes"
	"encoding/json"
	"fmt"
)

// checks if the Refresh200Response type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &Refresh200Response{}

// Refresh200Response struct for Refresh200Response
type Refresh200Response struct {
	Data AuthToken `json:"data"`
}

type _Refresh200Response Refresh200Response

// NewRefresh200Response instantiates a new Refresh200Response object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewRefresh200Response(data AuthToken) *Refresh200Response {
	this := Refresh200Response{}
	this.Data = data
	return &this
}

// NewRefresh200ResponseWithDefaults instantiates a new Refresh200Response object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewRefresh200ResponseWithDefaults() *Refresh200Response {
	this := Refresh200Response{}
	return &this
}

// GetData returns the Data field value
func (o *Refresh200Response) GetData() AuthToken {
	if o == nil {
		var ret AuthToken
		return ret
	}

	return o.Data
}

// GetDataOk returns a tuple with the Data field value
// and a boolean to check if the value has been set.
func (o *Refresh200Response) GetDataOk() (*AuthToken, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Data, true
}

// SetData sets field value
func (o *Refresh200Response) SetData(v AuthToken) {
	o.Data = v
}

func (o Refresh200Response) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o Refresh200Response) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["data"] = o.Data
	return toSerialize, nil
}

func (o *Refresh200Response) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"data",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err
	}

	for _, requiredProperty := range requiredProperties {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varRefresh200Response := _Refresh200Response{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varRefresh200Response)

	if err != nil {
		return err
	}

	*o = Refresh200Response(varRefresh200Response)

	return err
}

type NullableRefresh200Response struct {
	value *Refresh200Response
	isSet bool
}

func (v NullableRefresh200Response) Get() *Refresh200Response {
	return v.value
}

func (v *NullableRefresh200Response) Set(val *Refresh200Response) {
	v.value = val
	v.isSet = true
}

func (v NullableRefresh200Response) IsSet() bool {
	return v.isSet
}

func (v *NullableRefresh200Response) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableRefresh200Response(val *Refresh200Response) *NullableRefresh200Response {
	return &NullableRefresh200Response{value: val, isSet: true}
}

func (v NullableRefresh200Response) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableRefresh200Response) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
